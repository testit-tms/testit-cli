...
from testit_api_client import ApiClient as TmsClient, CreateEmptyTestRunApiModel, TestResultShortResponse, \
    AutoTestApiResult, UpdateEmptyTestRunApiModel
...
from testit_api_client.models import AttachmentPutModel
...
class ApiClient:
    ...
    def create_test_run(self, project_id: str, name: str) -> TestRun:
        ...
        return Converter.test_run_v2_get_result_to_test_run(test_run)

    def update_test_run(self, test_run: TestRun):
        """Function updates test run."""
        model: UpdateEmptyTestRunApiModel = Converter.test_run_to_update_empty_request(test_run)
    ...
    def get_test_run(self, test_run_id: str) -> Converter.test_run_v2_get_result_to_test_run:
    ...
    def get_autotests(self, model: Converter.project_id_and_external_id_to_autotests_search_post_request)\
        ...
        autotests: list[AutoTestApiResult] = self.__autotest_api.api_v2_auto_tests_search_post(auto_test_search_api_model=model)
    ...
    def create_autotest(self, model: Converter.test_result_to_create_autotest_request):
        ...
        response = self.__autotest_api.create_auto_test(model)
    ...
    def update_autotest(self, model: Converter.test_result_to_update_autotest_request):
        ...
        try:
            ...
            self.__autotest_api.update_auto_test(model)
        ...
        for attachment in attachments:
            if os.path.isfile(attachment):
                with open(attachment, "rb+") as file:
                    try:
                        attachment_response = self.__attachments_api.api_v2_attachments_post((file.name, file.read()))
    ...
    def get_test_results(
        ...
        """Function returns list of TestResultShortResponse."""
        logging.debug(f"Getting test results: {model}")

        test_results = self.__test_results_api.api_v2_test_results_search_post(model)
